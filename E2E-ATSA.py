...
# def twitter(mode="train"):
#     data_dir = f"dataset/new_data/Twitter/{mode}.raw"
#     out_dir = f"dataset/json/{mode}_en.json"
#     out_data = []
#     pos_num, neu_num, neg_num = 0, 0, 0
...
#             label = id2sent[polarity]
#             text = sentence.replace("$T$", aspect)
#             out_data.append({"text": text, "aspect": aspect, "label": label})
#             if label == "negative":
#                 neg_num += 1
#             elif label == "neutral":
#                 neu_num += 1
...
#     return [dataset_name, total_num, pos_num, neu_num, neg_num]


def lap14(mode="train"):
    data_dir = f"dataset/new_data/Lap14/{mode}_en.xml"
    out_dir = f"dataset/json/E2E-ATSA/lap14_{mode}_en.json"
